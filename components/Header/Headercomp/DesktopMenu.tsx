...
export default function DesktopMenu(props: { finishedLoading: boolean }) {
  return (
    <div className="font-mono text-xs md:flex hidden flex-row items-center space-x-8 ">
      ...
      >
        <ReactScrollLink to="Blogs" spy={true} smooth={true} offset={-100} duration={200}>
          &gt; 05. <span className="text-white hover:cursor-pointer hover:text-AAsecondary duration-300">Blogs</span>
        </ReactScrollLink>
      </motion.div>
      <a href={"/Gouravresume.pdf"} target={"_blank"} rel="noreferrer">
      <motion.button
        initial={{
          y: -40,
          opacity: 0,
