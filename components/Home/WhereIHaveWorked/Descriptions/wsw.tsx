import React from "react";
import ArrowIcon from "../../../Icons/ArrowIcon";
import { getTasksTextWithHighlightedKeyword } from "./taskAndType";

export default function WSW() {
  const tasks = [
    {
      text: "We created an app which will create awareness among the Govt School(6000 Approx) Students about Cyber World",
      keywords: ["Cyber World"],
    },
    {
      text: "We delivered seamless awareness among the students the importance of Cyber Knowledge in today's world",
      keywords: ["Cyber Knowledge"],
    },
    {
      text: "Interacted with more than 6000 students across the govt schools and made them aware about Cyberbullying, Cybercrimes, Virus, Malwares,etc",
      keywords: ["6000", "Cybercrimes", "Cyberbullying","Virus","Malwares"],
    },
  ];

  return (
    <>
      <div className="flex flex-col space-y-5 max-w-xl px-4 md:px-0">
      <div className="flex flex-col space-y-2">
          {/* Title */}
          <span className="text-gray-100 sm:text-lg text-sm font-Arimo tracking-wide">
          Cyber Ambassador Platform <span className="text-AAsecondary">@ React</span>
          </span>
          {/* Date */}
          <span className="font-mono text-xs text-gray-500">June 2024 - Jul 2024</span>
          <span
            className="font-mono text-xs text-AAsecondary hover:cursor-pointer"
            style={{ fontSize: "0.6rem" }}
           // set on click to open the website
           onClick={() => window.open("https://www.womensafetywing.telangana.gov.in/", "_blank")}
          >
            www.womensafetywing.telangana.gov.in
          </span>
        </div>
        <div className="flex flex-col space-y-4 sm:text-sm text-xs">
          {/* Tasks Description 1 */}
          {tasks.map((item, index) => {
            return (
              <div key={index} className="flex flex-row space-x-1">
                <ArrowIcon className={" h-5 w-4 text-AAsecondary flex-none"} />
                <span
                  className="text-gray-500 sm:text-sm text-xs"
                  dangerouslySetInnerHTML={{
                    __html: getTasksTextWithHighlightedKeyword(item.text, item.keywords),
                  }}
                ></span>
              </div>
            );
          })}
        </div>
      </div>
    </>
  );
}
